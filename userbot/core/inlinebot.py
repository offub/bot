...
OFFLOGO = "https://telegra.ph/file/52e682bdd3287289e456e.jpg"
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Offuserbot"):
            buttons = [
                (
            ...
            if IALIVE_PIC:
                OFF = [x for x in IALIVE_PIC.split()]
                PIC = list(OFF)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                OFF = [x for x in ALIVE_PIC.split()]
                PIC = list(OFF)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            buttons = []
            media = None
            offmedia = MEDIA_PATH_REGEX.search(markdown_note)
            if offmedia:
                media = offmedia.group(2)
                markdown_note = markdown_note.replace(offmedia.group(0), "")
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        moha = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    # ValueError: Could not find the input entity
                    moha = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                title="Troll Message",
                text=f"Only {moha} cannot access this message!",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {moha}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            _result = main_menu()
            result = builder.article(
                title="© OffUserbot Help",
                description="Help menu for OffUserbot",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if PM_PIC:
                OFF = [x for x in PM_PIC.split()]
                PIC = list(OFF)
                OFF_IMG = random.choice(PIC)
            else:
                OFF_IMG = None
            query = gvarstatus("pmpermit_text")
            if OFF_IMG and OFF_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    OFF_IMG,
                    title="Alive off",
                    text=query,
        ...
        buttons = [
            (
                Button.url("Source code", "https://github.com/offub/bot"),
                Button.url(
                    "Deploy",
        ...
        markup = event.client.build_reply_markup(buttons)
        photo = types.InputWebDocument(
            url=OFFLOGO, size=0, mime_type="image/jpeg", attributes=[]
        )
        text, msg_entities = await event.client._parse_message_text(
            "𝗗𝗲𝗽𝗹𝗼𝘆 𝘆𝗼𝘂𝗿 𝗼𝘄𝗻 𝗢𝗳𝗳𝗨𝘀𝗲𝗿𝗯𝗼𝘁.", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            ...
            url="https://github.com/offub/bot",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@offub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@offub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@offub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@offub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
