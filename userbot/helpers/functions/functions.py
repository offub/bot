...
async def fileinfo(file):
    ...
    off_json = json.loads(x)["media"]["track"]
    dic = {
        "path": file,
        "size": int(off_json[0]["FileSize"]),
        "extension": off_json[0]["FileExtension"],
    }
    try:
        if "VideoCount" or "AudioCount" or "ImageCount" in off_json[0]:
            dic["format"] = off_json[0]["Format"]
            dic["type"] = off_json[1]["@type"]
            if "ImageCount" not in off_json[0]:
                dic["duration"] = int(float(off_json[0]["Duration"]))
                dic["bitrate"] = int(int(off_json[0]["OverallBitRate"]) / 1000)
            if "VideoCount" or "ImageCount" in off_json[0]:
                dic["height"] = int(off_json[1]["Height"])
                dic["width"] = int(off_json[1]["Width"])
...
async def animator(media, mainevent, textevent=None):
    ...
    Off = await mainevent.client.download_media(media, Config.TEMP_DIR)
    file = await fileinfo(Off)
    h = file["height"]
    w = file["width"]
    ...
    await runcmd(
        f"ffmpeg -to 00:00:02.900 -i '{Off}' -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(Off)
    return "animate.webm"
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    off = await sticcers[c_lick].click("me", hide_via=True)
    if off:
        await borg.send_file(int(chat_id), off, reply_to=reply_to_id)
        await off.delete()


async def make_inline(text, borg, chat_id, reply_to_id):
    offinput = f"Inline buttons {text}"
    results = await borg.inline_query(Config.TG_BOT_USERNAME, offinput)
...
def higlighted_text(
    ...
    for pic_no, list_text in enumerate(texts):
        ...
        output_img = f"./temp/off{pic_no}.jpg"
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    off = await sticcers[0].click("me", hide_via=True)
    if off:
        await bot.send_file(int(chat_id), off, reply_to=reply_to_id)
        await off.delete()
