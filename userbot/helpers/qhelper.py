...
def file_check():
    ...
    if not os.path.exists(special):
        urllib.request.urlretrieve(
            "https://github.com/offub/bot-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            special,
        )
    ...
    if replied:
        ...
        if reply and reply.sticker:
            ...
            if sticker.endswith(("tgs")):
                ...
                stdout, stderr = (await _offutils.runcmd(cmd))[:2]
        ...
        canvas.paste(middle, (pfpbg.width, top.height))
        canvas.paste(bottom, (pfpbg.width, top.height + middle.height))
        canvas = await offdoctype(docname, docsize, doctype, canvas)
        y = 80 if text else 0
    else:
        canvas.paste(pfpbg, (0, 0))
        canvas.paste(top, (pfpbg.width, 0))
...
async def offdoctype(name, size, htype, canvas):
