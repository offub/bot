...
class OffGloballist(BASE):
    __tablename__ = "offglobal_list"
    ...
    def __repr__(self):
        return "<Off global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, OffGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


OffGloballist.__table__.create(checkfirst=True)

OFFGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with OFFGLOBALLIST_INSERTION_LOCK:
        broadcast_group = OffGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with OFFGLOBALLIST_INSERTION_LOCK:
        if broadcast_group := SESSION.query(OffGloballist).get(
...
def is_in_list(keywoard, group_id):
    with OFFGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(OffGloballist).get((keywoard, str(group_id)))
...
def del_keyword_list(keywoard):
    with OFFGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(OffGloballist.keywoard)
            .filter(OffGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(OffGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(OffGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(OffGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(OffGloballist).all()
        for x in all_groups:
