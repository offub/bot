...
from ..helpers.utils import _offtools, _offutils, _format, install_pip, reply_id
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod.Config = Config
        mod._format = _format
        mod.tgbot = offub.tgbot
        mod.sudo_cmd = sudo_cmd
        mod.CMD_HELP = CMD_HELP
        mod.reply_id = reply_id
        mod.admin_cmd = admin_cmd
        mod._offutils = _offutils
        mod._offtools = _offtools
...
def remove_plugin(shortname):
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(offub._event_builders))):
            ev, cb = offub._event_builders[i]
            if cb.__module__ == name:
                del offub._event_builders[i]
